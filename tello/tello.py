...
class Tello:
    ...
    def get_udp_video_address(self):
        return (
            "udp://@" + self.VS_UDP_IP + ":" + str(self.VS_UDP_PORT)
    ...
    def send_command_with_return(
        self, command, printinfo=True, timeout=RESPONSE_TIMEOUT
    ):
    ...
    def send_control_command(self, command, timeout=RESPONSE_TIMEOUT):
        ...
        return self.return_error_on_send_command(
            command, response, self.enable_exceptions
        )
    ...
    def send_rc_control(
        ...
        """
        if (
            int(time.time() * 1000) - self.last_rc_control_sent
            ...
            return self.send_command_without_return(
                "rc %s %s %s %s"
                % (
                    self.round_to_100(left_right_velocity),
